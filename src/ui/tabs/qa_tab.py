...
def create_qa_tab():
    ...
    if st.session_state.get('processing_question', False):
        start_time = st.session_state.get('processing_start_time', 0)
    ...
    # Ask question button with processing state management
    if st.button("🔍 Ask Question", type="primary", disabled=not question.strip()):
        if question.strip():
            # Check if already processing
            if st.session_state.get('processing_question', False):
                st.warning("⏳ Please wait, processing your previous question...")
            else:
